...
int main()
{
  ...
  vector<double> init_deltas;
  init_params.push_back(k_p);
  init_deltas.push_back(0.001);
  init_params.push_back(k_i);
  ...
  h.onMessage([&ts, &pid, &twd, &cruise_pid, &ref_speed](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  if (steer_value > 0)
		...
		steer_value = max(steer_value, -deg2rad(25));

	  /*
	   * Maintain speed using PID controller
	   */
